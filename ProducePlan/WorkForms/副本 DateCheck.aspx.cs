...
public partial class WorkForms_DateCheck : System.Web.UI.Page
{
    ...
    protected void Bind(bool isFirstPage)
    {
        ...
        string sql =
            " select * from (select substring(inv.[cInvcCode],1,4) 大分类编码 , inv.[cInvcCode] 分类编码,a.dPODate 订单日期,b.cupsocode 销售订单号,a.cpoid 采购订单号,b.[cInvCode] 存货编码, inv.cInvName 存货名称,b.dArriveDate 计划到货日期,b.iQuantity 订单数量,isnull(b.iarrQty,0) 累计到货数量, datediff(day,getdate(),b.dArriveDate) as 天数" +
            ...
            " and  datediff(day,getdate(),b.dArriveDate)<=4                                                                                                                                                                                  " +
            //" and  datediff(day,getdate(),b.dArriveDate)>=0                                                                                                                                                                                  " +
            "                                                                                                                                                                                                                                " +
            " union    all                                                                                                                                                                                                                      " +
            " select substring(inv.[cInvcCode],1,4) 大分类编码 ,inv.[cInvcCode] 分类编码,a.dPODate 订单日期,b.cupsocode 销售订单号,a.cpoid 采购订单号,b.[cInvCode] 存货编码, inv.cInvName 存货名称,b.dArriveDate 计划到货日期,b.iQuantity 订单数量,isnull(b.iarrQty,0) 累计到货数量 , datediff(day,getdate(),b.dArriveDate) as 天数              " +
            " from [PO_Podetails] b left join [PO_Pomain] a  on a.poid=b.poid left join [Inventory] inv on inv.[cInvCode]=b.[cInvCode]                                                                                                       " +
            " where b.iQuantity > ISNULL(b.iarrQty,0)  and a.dPODate>'2015-01-01' and len(inv.[cInvcCode])>=4 and b.cbCloser is null and                                                                                                                                       " +
            " (left(inv.[cInvcCode],4)='0104')                                                                                                                                                                                               " +
            " and  datediff(day,getdate(),b.dArriveDate)<=5                                                                                                                                                                                  " +
            //" and  datediff(day,getdate(),b.dArriveDate)>=0                                                                                                                                                                                  " +
            "                                                                                                                                                                                                                                " +
            " union     all                                                                                                                                                                                                                     " +
            "                                                                                                                                                                                                                                " +
            ...
            " and  datediff(day,getdate(),b.dArriveDate)<=7                                                                                                                                                                                  " +
            //" and  datediff(day,getdate(),b.dArriveDate)>=0" +
            ") as bigTable                                                                                                                                                                     ";
sql += " where 1=1 ";
