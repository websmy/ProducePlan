...
public partial class WorkForms_baoxiaochaxun : System.Web.UI.Page
{
    private const string fromthedate = "2017-10-01";
    protected void Page_Load(object sender, EventArgs e)
    {

        if (!Page.IsPostBack)
        {
            //txtStartDate.Text = fromthedate;
    ...
    //    "              wf.WORKPROCESS_ID ,                                                            " +
    //    "              whir$t3034_f3307 部门 ,                                                        " +
    //    "              whir$t3034_f3308 报销人 ,                                                      " +
    //    "              whir$t3034_f4580 项目 ,                                                        " +
    //    "              whir$t3035_f3320 事项 ,                                                        " +
    //    "              whir$t3035_f3322 金额                                                          " +
    //    "    FROM      [ezoffice].[WF_WORK] wf                                                        " +
    //    "              INNER JOIN whir$t3034 a ON wf.WORKRECORD_ID = a.whir$t3034_id                  " +
    //    "              LEFT JOIN whir$t3035 b ON a.whir$t3034_id = b.whir$t3035_FOREIGNKEY            " +
    //    "    WHERE     wf.WORKSTATUS != -1                                                            " +
    //    "              AND wf.WORKSTATUS != -2                                                        " +
    //    "              AND CONVERT(VARCHAR(7), a.whir$t3034_f3306, 120) >= '" + fromthedate + "'               " +
    //    "              " +
    //    "  )                                                                                          " +
    //    "  UNION                                                                                      " +
    //    "  ( SELECT  DISTINCT                                                                         " +
    //    "              wf.WORKPROCESS_ID ,                                                            " +
    //    "              whir$t3066_f3604 部门 ,                                                        " +
    //    "              whir$t3066_f3605 报销人 ,                                                      " +
    //    "              whir$t3066_f4579 项目 ,                                                        " +
    //    "              whir$t3067_f3617 事项 ,                                                        " +
    //    "              whir$t3067_f3619 金额                                                          " +
    //    "    FROM      [ezoffice].[WF_WORK] wf                                                        " +
    //    "              INNER JOIN whir$t3066 a ON wf.WORKRECORD_ID = a.whir$t3066_id                  " +
    //    "              LEFT JOIN whir$t3067 b ON a.whir$t3066_id = b.whir$t3067_FOREIGNKEY            " +
    //    "    WHERE     wf.WORKSTATUS != -1                                                            " +
    //    "              AND wf.WORKSTATUS != -2                                                        " +
    //    "              AND CONVERT(VARCHAR(10), a.whir$t3066_f3606, 120) >= '" + fromthedate + "'               " +
    //    "              " +
    //    "  )                                                                                          " +
    //    "  UNION                                                                                      " +
    //    "  ( SELECT  DISTINCT                                                                         " +
    //    "              wf.WORKPROCESS_ID ,                                                            " +
    //    "              whir$t3205_f5159 部门 ,                                                        " +
    //    "              whir$t3205_f5160 报销人 ,                                                      " +
    //    "              whir$t3205_f5161 项目 ,                                                        " +
    //    "              whir$t3035_f3320 事项 ,                                                        " +
    //    "              whir$t3035_f3322 金额                                                          " +
    //    "    FROM      [ezoffice].[WF_WORK] wf                                                        " +
    //    "              INNER JOIN whir$t3205 a ON wf.WORKRECORD_ID = a.whir$t3205_id                  " +
    //    "              LEFT JOIN whir$t3035 b ON a.whir$t3205_id = b.whir$t3035_FOREIGNKEY            " +
    //    "    WHERE     wf.WORKSTATUS != -1                                                            " +
    //    "              AND wf.WORKSTATUS != -2                                                        " +
    //    "              AND CONVERT(VARCHAR(10), a.whir$t3205_f5158, 120) >= '" + fromthedate + "'               " +
    //    "                " +
    //    "  )                                                                                          ";
    ...
    //    DataSet ds = SQLHelperOA.Query(sql);
    ...
    //    dataView.Sort = "报销人 ";
    //    DataTable dataTableDistinct1 = dataView.ToTable(true, "报销人"); //注      
    ...
    //    DropDownList1.DataValueField = "报销人";
    ...
    {
        ViewState["mothday"] = ddlYear.Text + "-" + ddlMonth.Text;
        //ViewState["b"] = 0;
        ...
        string sqlFirst1 = "    SELECT    zuiwaimian.部门,SUM(zuiwaimian.金额) 金额,b.部门金额,  ( b.部门金额  - SUM(zuiwaimian.金额)) 差值 FROM  ( ";
        string sqlFirst2 = "  select  SUM(金额) from  ( ";
      ...
      " ( SELECT  DISTINCT" +
        ...
        "    WHERE     wf.WORKSTATUS != -1                                                            " +
        "              AND wf.WORKSTATUS != -2                                                        " +
        "              AND CONVERT(VARCHAR(7), a.whir$t3034_f3306, 120) = '" + ViewState["mothday"] + "'               " +
        "                                                       " +
        "  )                                                                                          " +
        "  UNION                                                                                      " +
        "  ( SELECT  DISTINCT                                                                         " +
        ...
        "    WHERE     wf.WORKSTATUS != -1                                                            " +
        "              AND wf.WORKSTATUS != -2                                                        " +
        "              AND CONVERT(VARCHAR(7), a.whir$t3066_f3606, 120) = '" + ViewState["mothday"] + "'    " +
        "    " +
        "  )                                                                                          " +
        "  UNION                                                                                      " +
        "  ( SELECT  DISTINCT                                                                         " +
        ...
        "    WHERE     wf.WORKSTATUS != -1                                                            " +
        "              AND wf.WORKSTATUS != -2                                                        " +
        "              AND CONVERT(VARCHAR(7), a.whir$t3205_f5158, 120) = '" + ViewState["mothday"] + "'   " +
        "      " +
        "  )                                                                                          " +
      ...
      "  ";
        ...
        string sqlAdded1 = "     left join opendatasource ('SQLOLEDB','" + GetConnectionString.iGetConn1 + "')." + vDataBaseName +
          "   .dbo.[财务预算] b on zuiwaimian.部门 = b.部门名称   " +
                           " GROUP BY zuiwaimian.部门,b.部门名称,b.部门金额";
        ...
        //    {
        //        sqlAdded += " and (zuiwaimian.报销人 is null or zuiwaimian.报销人='') ";
        //    }
        //    else
        //    {
        //        sqlAdded += " and zuiwaimian.报销人='" + DropDownList1.SelectedValue + "'";
        //    }
        ...
        //    {
        //        sqlAdded += " and (zuiwaimian.部门 is null or zuiwaimian.部门='') ";
        //    }
        //    else
        //    {
        //        sqlAdded += " and zuiwaimian.部门='" + DropDownList3.SelectedValue + "'";
        //    }
        ...
        //}
        string sql = sqlFirst1 + sqlAdded + sqlAdded1;
        ...
        string sqlSum = sqlFirst2 + sqlAdded;


        //=============start
        DataSet ds11 = SQLHelperOA.Query(sqlSum);
        double d到期应收款 = 0;
        ...
        //        DataSet ds1 = SQLHelperOA.Query(sql13);
        //        double itemp = Convert.ToDouble(ds1.Tables[0].Rows[0][0]);
        //        itemp = itemp - 信用额度;
        //        if (itemp > 0)
        //        {
        //            // to do strat
        //            DataTable dtTemp = SQLHelperOA.Query("select * from 质保金 where [客户编码] ='" + ds11.Tables[0].Rows[i]["客户编码"] + "'").Tables[0];
        ...
        //                           "' and  客户编码='" + ds11.Tables[0].Rows[i]["客户编码"] + "'";
        //                DataSet ds2 = SQLHelperOA.Query(sql1);
        ...
        if (!String.IsNullOrEmpty(ds11.Tables[0].Rows[0][0].ToString()))
        {
            Label7.Text = Convert.ToDouble(ds11.Tables[0].Rows[0][0]).ToString("n");
        }
        else
        {
            Label7.Text = 0.ToString("n");
        }
    ...
    protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowIndex > -1)
        {
            ...
            HiddenField 部门 = e.Row.FindControl("部门") as HiddenField;
            ...
            //        labelsection1.Text = Convert.ToInt32(SQLHelperOA.Query(sql).Tables[0].Rows[0][0]).ToString("n");
            ...
            //        labelsection2.Text = Convert.ToInt32(SQLHelperOA.Query(sql).Tables[0].Rows[0][0]).ToString("n");
            ...
            //        labelsection3.Text = Convert.ToInt32(SQLHelperOA.Query(sql).Tables[0].Rows[0][0]).ToString("n");
            ...
            //        labelsection4.Text = Convert.ToInt32(SQLHelperOA.Query(sql).Tables[0].Rows[0][0]).ToString("n");
            //        //========================
            ...
            //        labelsection5.Text = Convert.ToInt32(SQLHelperOA.Query(sql).Tables[0].Rows[0][0]).ToString("n");
            ...
            //        labelsection6.Text = Convert.ToInt32(SQLHelperOA.Query(sql).Tables[0].Rows[0][0]).ToString("n");
            ...
            //        labelsection7.Text = Convert.ToInt32(SQLHelperOA.Query(sql).Tables[0].Rows[0][0]).ToString("n");

            //        sql = " select  isnull(sum(未核销金额),0) from 收款单表 where [单据日期]<='" + DateTime.Now.Date.AddDays(-1826) + "' and [客户编码]='" + 客户编码.Value + "' ";
            //        labelsection8.Text = Convert.ToInt32(SQLHelperOA.Query(sql).Tables[0].Rows[0][0]).ToString("n");
            //        //end 2014-08-02
            //    }
            //    else
            //    {
            ...
            //              "' and  客户编码='" + 客户编码.Value + "'";
            //        DataSet ds1 = SQLHelperOA.Query(sql);
            //        double itemp = Convert.ToDouble(ds1.Tables[0].Rows[0][0]);
            ...
            //        {
            //            // to do strat
            //            DataTable dtTemp = SQLHelperOA.Query("select * from 质保金 where [客户编码] ='" + 客户编码.Value + "'").Tables[0];
            //            if (dtTemp.Rows.Count == 1)
            //            {
            //                DateTime dttemp = System.DateTime.Now.AddYears(-Convert.ToInt32(dtTemp.Rows[0]["期限"]));
            //                string sql1 = " select isnull(sum(质保金未核销),0)    from  [销售发票] where [开票日期] <='" + dttemp.ToShortDateString() +
            ...
            //if (SQLHelperOA.Query(" select * from SetColor where cSOCode='"+cSOCode.Value+"' ").Tables[0].Rows.Count>0)
            //{
            ...
            HyperLink1.Attributes.Add("onclick",
                                          "openwin('Mom4.aspx','Mom4','" + 部门.Value + "','650','600','" +
                                          "" + ViewState["mothday"] + "','" + "" + "','" +
                                          "" + "','" +
                                          "" + "','" + "" + "','" + "" + "' )");
    ...
    {
        if (DropDownList1.Text.Equals("请选择"))//HasFile用来检查FileUpload是否有指定文件
        {
            Response.Write("<script>alert('请您选择年')</script> ");
            return;//当无文件时,返回
        }

        if (DropDownList2.Text.Equals("请选择"))
        {
            Response.Write("<script>alert('请您选择月')</script> ");
            return;//当无文件时,返回
        }
        ...
        if (!IsXls.Equals(".xlsx") && !IsXls.Equals(".xls"))
        {
        ...
        string savePath = Server.MapPath(("uploadfiles\\") + str + filename);//Server.MapPath 获得虚拟服务器相对路径
        ...
        else
        {
            ...
            for (int i = 0; i < dr.Length; i++)
            {
                ...
                string 部门ID = dr[i]["部门ID"].ToString();
                string 部门名称 = dr[i]["部门名称"].ToString();
                string 部门金额 = dr[i]["部门金额"].ToString();
                //string customername = dr[i]["内容摘要"].ToString();

                try
                {
                    var uuid = Guid.NewGuid().ToString();
                    string flag = DropDownList1.Text + DropDownList2.Text;
                    
                    string sql = "select * from [财务预算] where flag='"+flag+"'        ";
                    if (SQLHelper1.Exists(sql))
                    {
                        //MessageBox.Show(this, "材料编码: " + GridView2.Rows[i].Cells[2].Text + " 已经存在,请先将其删除掉.");
                        //return;
                        sql = string.Format("    update 财务预算 set 部门金额 = '{0}'  where flag=  '{1}'  and   部门ID=  '{2}'      ", 部门金额, flag, 部门ID );
                        SQLStringList.Add(sql);

                    }
                    else
                    {
                        sql = string.Format("insert into 财务预算(部门ID,部门名称,部门金额,flag) values('{0}','{1}','{2}','{3}')", 部门ID,部门ID+";"+ 部门名称, 部门金额, flag);
                        SQLStringList.Add(sql);
                    }
                    ...
                    //string sql = "";
                    //int count = SQLHelperOA.ExecuteSql(sql);
                    //if (count > 0)
                    //    successly++;
